...
class Patient:
    ...
    def is_prescribed(self, medication: str) -> bool:
        ...
        for visit in self.history:
            if visit.prescribed == medication:
                return True
        return False
    ...
    def followups(self, month: str) -> List[datetime.date]:
        ...
        for visit in self.history:
            if visit.followup_date is not None:
                if visit.followup_date.month == MONTH_ABBREV.index(month) + 1:
                    followup_dates.append(visit.followup_date)
    ...
    def prescribed_after(self, date: datetime.date) -> List[str]:
        ...
        for visit in self.history:
            if visit.date >= date:
                if visit.prescribed is not None or visit.prescribed != '':
                    meds.append(visit.prescribed)
        return meds
...
class Hospital:
            ...
            for line in file:
                ...
                if x:
                    for doc in self.doctors:
                        if doc.name == info:
                            curr_doc = doc
                            x = False  # think about putting a break
                            break
                elif info != '':
        ...
        for patient in self.patients:
            for visit in patient.history:
                if visit.followup_date is not None and \
                        date <= visit.followup_date <= followup_date and \
                        patient not in followup_list:
    ...
    def patients_seen(self, doctor: Doctor, start_date: datetime.date,
        ...
        for day in range(0, delta_day + 1):
            curr_date = start_date + datetime.timedelta(day)
            if self.admissions.get(curr_date) is not None:
                for visit in self.admissions[curr_date]:
                    if visit.doctor_id == doctor.id and visit.patient_id \
                            not in patient_list:
                        unique_visits += 1
                        patient_list.append(visit.patient_id)
    ...
    def busiest_doctors(self, start_date: datetime.date,
        ...
        num_attend_list = []
        for doc in self.doctors:
            num_attended = self.patients_seen(doc, start_date, end_date)
            num_attend_list.append(num_attended)
        most_attended = max(num_attend_list)
        for i in range(len(num_attend_list)):
            if num_attend_list[i] == most_attended:
                doctors.append(self.doctors[i])
        return doctors
    ...
    def coverage(self, bob: Doctor, alice: Doctor) -> List[datetime]:
        ...
        # ===== #
        bob_work_days = []
        for month in bob.schedule.keys():
            bob_work_days.extend(bob.schedule[month])
        coverage = []
        alice_sick_days = self.sick_days(alice)
        for date in alice_sick_days:
            if self.attendance.get(date) is not None:
                if bob.name in self.attendance[date] and date not in \
                        bob_work_days:
    ...
    def sick_days(self, doctor: Doctor) -> List[datetime.date]:
        ...
        work_days = []
        sick_days = []
        for month in doctor.schedule.keys():
            work_days.extend(doctor.schedule[month])
        for date in work_days:
            if doctor.name not in self.attendance[date]:
                sick_days.append(date)
        return sick_days
    ...
    def attended_to(self, patient: Patient) -> List[Doctor]:
        ...
        for doc in self.doctors:
            for visit in patient.history:
                if visit.doctor_id == doc.id and doc not in doctors:
                    doctors.append(doc)
        return doctors
    ...
    def prescribed_rate(self, doctor: Doctor, medication: str) -> float:
        ...
        # TO DO #
        # ===== #
        prescribed_med_num = 0
        prescribed_num = 0
        for patient in self.patients:
            for visit in patient.history:
                if visit.doctor_id == doctor.id:
                    prescribed_num, prescribed_med_num = _med_prescribe_numbers(
                        visit, medication, prescribed_num, prescribed_med_num)
        if prescribed_num == 0:
            return 0.0
        return (prescribed_med_num / prescribed_num) * 100


def _med_prescribe_numbers(visit: HospitalVisit, medication: str,
                           prescribed_num: int,
                           prescribed_med_num: int) -> List[int, int]:
    """
    Returns the number of prescribed medicines and the number of
    prescriptions of a certain medicine a doctor prescribed in a list of ints.
    This is a helper function to the method prescribed_rate. This is a private
    method and should never be called on its own.

    """
    if visit.prescribed is not None and visit.prescribed != '':
        prescribed_num += 1
        if visit.prescribed == medication:
            prescribed_med_num += 1
    return [prescribed_num, prescribed_med_num]
